...
public class CertificateChainStatus {
    public static final String[] KEYSTORE_GOOGLE_PUBLIC_KEYS = {
            "-----BEGIN PUBLIC KEY-----\n" +
    ...
    public CertificateChainStatus() {
        ...
        this.rootStatus = RootStatus.Unknown;
    ...
    public CertificateChainStatus(X509Certificate @NotNull [] chain) {
        ...
        if (chain.length > 1) {
            rootStatus = checkRootStatus(chain[chain.length - 1]);
        } else {
            rootStatus = RootStatus.Unknown;
        }

        if (rootStatus == RootStatus.SignedByGoogle) {
            revocationStatus = checkRevocationStatus(chain);
        } else {
            revocationStatus.setStatus(RevocationStatus.Status.Unknown, RevocationStatus.Reason.NotGoogleRoot);
        }
    }

    private static RootStatus checkRootStatus(X509Certificate root) {
        try {
            KeyFactory keyFactory = KeyFactory.getInstance("RSA");

            for (String key_pem : KEYSTORE_GOOGLE_PUBLIC_KEYS) {
                String key_b64 = key_pem.replace("-----BEGIN PUBLIC KEY-----", "").replaceAll("\n", "").replace("-----END PUBLIC KEY-----", "");
                X509EncodedKeySpec keySpec = new X509EncodedKeySpec(Base64.getDecoder().decode(key_b64));
                PublicKey key = keyFactory.generatePublic(keySpec);
                if (key.equals(root.getPublicKey())) {
                    return RootStatus.SignedByGoogle;
                }
            }
            ...
            return RootStatus.NotSignedByGoogle;
        } catch (Exception e) {
            return RootStatus.Unknown;
        }
    }

    private static RevocationStatus checkRevocationStatus(X509Certificate[] chain) {
    ...
    public String toString(Context context) {
        ...
        joiner.add(context.getResources().getString(R.string.certificate_signed_by_google_root) + ": " + rootStatus.toString(context));
